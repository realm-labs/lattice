...
fn push_gateway_route_table(rust: &mut String, methods: &[RpcMethodSpec]) {
    ...
    for method in gateway_methods {
        ...
        rust.push_str(&format!(
            "{}{}GatewayBinding::binding().route_spec()",
            method.service_name, method.method_name
        ));
        rust.push_str(")?;\n");
    }
...
"#
    ...
    fn gateway_route_table_registration_is_generated_from_method_metadata() {
        ...
        second.request_type = "JoinRoomRequest".into();
        second.reply_type = "JoinRoomReply".into();
        ...
        assert!(
            generated.rust.contains(
                "table.register(RoomRpcJoinRoomGatewayBinding::binding().route_spec())?;"
            )
        );
    }
