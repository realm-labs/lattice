...
pub fn generate_rpc_bindings_with_direct_link_streams(
    ...
    validate_direct_link_streams(direct_link_streams)?;
    ...
    rust.push_str(
        "#[allow(unused_imports)]\nuse lattice_gateway::{ClientFrame, GatewayError, GatewayRouteTable, MessageRouter};\n",
    );
    rust.push_str(
        "#[allow(unused_imports)]\nuse lattice_placement::{EndpointLease, EndpointRpcTransport};\n",
    );
    rust.push_str(
        "#[allow(unused_imports)]\nuse lattice_rpc::{ActorRpcAdapter, RoutedEnvelope, Rpc, RpcError, RpcRequest, RpcServerSecurity, RoutedRequest, ShardedRpcCore, TonicEndpointChannelPool, TypedRpcClient};\n",
    );
    ...
    push_generated_direct_link_streams(&mut rust, direct_link_streams);
...
pub fn generate_direct_link_stream_bindings(
    ...
    validate_direct_link_streams(streams)?;
    let mut rust = String::new();
    rust.push_str("// @generated by lattice-codegen\n");
    push_generated_direct_link_streams(&mut rust, streams);
    Ok(GeneratedRpcBindings { rust })
}
...
fn validate_direct_link_streams(
    streams: &[GeneratedDirectLinkStreamSpec],
) -> Result<(), CodegenError> {
...
fn push_generated_direct_link_streams(
    rust: &mut String,
    streams: &[GeneratedDirectLinkStreamSpec],
) {
    for stream in streams {
        ...
        for message in &stream.messages {
            rust.push_str("                lattice_core::DirectLinkMessageDescriptor {\n");
            rust.push_str(&format!(
                "                    message_id: lattice_core::DirectLinkMessageId({}),\n",
                message.message_id
            ));
            rust.push_str(&format!(
                "                    proto_full_name: <{} as lattice_core::DirectLinkMessage>::PROTO_FULL_NAME.to_string(),\n",
                message.rust_type
            ));
            rust.push_str(&format!(
        ...
        rust.push_str("            handle: &lattice_actor::ActorHandle<A>,\n");
...
fn push_typed_client(rust: &mut String, methods: &[&RpcMethodSpec]) {
    ...
    for method in methods {
        if method.route_key_from_request {
            ...
            rust.push_str(&format!(
                "        pub async fn {method}(&self, route_key: RouteKey, req: {request}) -> Result<{reply}, lattice_rpc::RpcError> {{\n",
                method = lower_camel_to_snake(&method.method_name),
                request = method.request_type,
                reply = method.reply_type
            ));
            rust.push_str(
                "            self.inner.core().call_routed(lattice_rpc::RoutedEnvelope::new(\n",
            );
            rust.push_str("                req,\n");
            rust.push_str(&format!(
...
fn push_method_module(rust: &mut String, method: &RpcMethodSpec) {
    ...
    if method.gateway_msg_id.is_some() {
        rust.push_str(
            "        use lattice_gateway::{ClientFrame, GatewayError, GatewayRouteContext, GatewayRouteSpec, MessageRouter};\n",
        );
...
fn push_gateway_dispatcher(
    ...
    rust.push_str("    }\n\n");
    rust.push_str(
        "    pub async fn dispatch_with_context<R>(&self, frame: ClientFrame, router: &mut R, context: &lattice_gateway::GatewayRouteContext) -> Result<ClientFrame, GatewayError>\n",
    );
    rust.push_str("    where\n        R: MessageRouter,\n    {\n");
