...
use crate::{
    coordinator::{
        ActorGroupHello, CoordinatorDelta, MemberEvent, MemberHello, MemberRecord,
        MemberRemovalReason, NodeLoadReport, ShardLoadReport, SnapshotBegin, SnapshotChunk,
...
impl PlacementControlScopeWire {
    ...
    fn into_scope(self) -> Result<CoordinatorScope, PlacementControlError> {
        ...
        {
            PlacementControlScopeKindWire::Membership if self.actor_group.is_empty() => {
                Ok(CoordinatorScope::Cluster)
            }
            PlacementControlScopeKindWire::Placement => ActorGroupId::new(self.actor_group)
...
impl SnapshotVersionWire {
    fn from_version(version: &SnapshotVersion) -> Self {
        match version {
            ...
            SnapshotVersion::Placement(version) => Self {
                ...
                actor_group: version.group.as_str().to_owned(),
                coordinator_term: version.term.get(),
                revision: version.revision.get(),
            },
        }
    ...
    fn into_version(self) -> Result<SnapshotVersion, PlacementControlError> {
        ...
        match SnapshotVersionKindWire::try_from(self.kind)
            .map_err(|_| PlacementControlError::Codec)?
        {
            SnapshotVersionKindWire::Membership if self.actor_group.is_empty() => Ok(
                SnapshotVersion::Membership(MembershipVersion::new(term, revision)),
            ),
            SnapshotVersionKindWire::Placement => {
                let group = ActorGroupId::new(self.actor_group)
                    .map_err(|_| PlacementControlError::Codec)?;
...
/// Bounded receiver directory used by logic nodes with independent group sessions.
...
mod tests {
    ...
    fn control_generation_round_trips_and_rejects_oversize() {
        ...
        let scope = CoordinatorScope::Group(ActorGroupId::new("control-test").unwrap());
    ...
    #[test]
    fn drain_confirmation_round_trips_for_each_scope() {
        for scope in [
            CoordinatorScope::Cluster,
            CoordinatorScope::Group(ActorGroupId::new("drain").unwrap()),
        ] {
    ...
    #[test]
    fn configured_placement_group_hello_round_trips() {
        ...
        .unwrap();
        let hello = ActorGroupHello::builder(
            NodeKey {
        ...
        .build();
        let command = PlacementControlCommand::ActorGroupHello(hello);
    ...
    fn snapshot_chunks_use_the_dedicated_protobuf_variant() {
        let scope = CoordinatorScope::Group(ActorGroupId::new("player").unwrap());
    ...
    #[test]
    fn control_stream_ids_are_stable_and_group_scoped() {
        let membership = control_stream_id(&CoordinatorScope::Cluster);
        let world = control_stream_id(&CoordinatorScope::Group(
            ActorGroupId::new("world").unwrap(),
        ));
        let player = control_stream_id(&CoordinatorScope::Group(
