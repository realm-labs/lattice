...
use lattice_model::{
    ...
    cluster::{ActorGroupId, ConfigFingerprint, EntityType, NodeIncarnation, SingletonKind},
};
...
impl SingletonConfig {
    pub fn new(group: ActorGroupId, kind: SingletonKind, protocol_id: ProtocolId) -> Self {
        let mut canonical = Vec::new();
        canonical.extend_from_slice(&(group.as_str().len() as u32).to_be_bytes());
    ...
    pub fn validate(&self) -> bool {
        Self::new(self.group.clone(), self.kind.clone(), self.protocol_id).config_fingerprint
            == self.config_fingerprint
    }
}
...
pub struct ClusterLeaderGuard {
    record: LeaderRecord,
}

impl ClusterLeaderGuard {
    pub fn new(record: LeaderRecord) -> Result<Self, CoordinatorError> {
        if record.scope != CoordinatorScope::Cluster {
...
/// Exact lease-backed actor-group leadership identity.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GroupLeaderGuard {
    record: LeaderRecord,
}
...
impl ExactLeaderGuard for ClusterLeaderGuard {
    fn record(&self) -> &LeaderRecord {
        &self.record
    }
}

impl ExactLeaderGuard for GroupLeaderGuard {
    fn record(&self) -> &LeaderRecord {
        &self.record
    }
}

impl GroupLeaderGuard {
    pub fn new(record: LeaderRecord) -> Result<Self, CoordinatorError> {
        if !matches!(record.scope, CoordinatorScope::Group(_)) {
            return Err(CoordinatorError::InvalidLeader);
        }
        record.validate()?;
        Ok(Self { record })
    ...
    pub fn group(&self) -> &ActorGroupId {
        let CoordinatorScope::Group(group) = &self.record.scope else {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ActorGroupHello {
    pub node: NodeKey,
    pub group: ActorGroupId,
    pub group_config_fingerprint: ConfigFingerprint,
...
impl ActorGroupHello {
    pub fn builder(
        node: NodeKey,
        group: ActorGroupId,
        capacity_units: u64,
    ) -> ActorGroupHelloBuilder {
        ActorGroupHelloBuilder {
            node,
            group,
            capacity_units,
    ...
    pub fn validate(&self, limits: &SessionLimits) -> Result<(), CoordinatorError> {
        ...
        if self.capacity_units == 0
            || self.group_config_fingerprint != placement_group_fingerprint(&self.group)
        ...
        if self.singleton_configs.iter().any(|config| {
            !config.validate()
                || config.group != self.group
                || !singleton_kinds.insert((config.group.clone(), config.kind.clone()))
...
pub struct ActorGroupHelloBuilder {
    node: NodeKey,
    group: ActorGroupId,
    capacity_units: u64,
...
impl ActorGroupHelloBuilder {
    ...
    pub fn build(self) -> ActorGroupHello {
        let group_config_fingerprint = placement_group_fingerprint(&self.group);
        ActorGroupHello {
            node: self.node,
            group: self.group,
            group_config_fingerprint,
...
fn placement_group_fingerprint(group: &ActorGroupId) -> ConfigFingerprint {
...
impl ActorGroupState {
    pub fn new(group: ActorGroupId) -> Self {
        Self {
            group,
            version: None,
    ...
    pub fn install(&mut self, snapshot: SnapshotInstall) -> Result<(), ActorGroupStateError> {
        let SnapshotVersion::Placement(version) = snapshot.version else {
            return Err(ActorGroupStateError::UnexpectedSnapshot);
        };
        if version.group != self.group {
            return Err(ActorGroupStateError::GroupMismatch);
        }
        if self.version.as_ref().is_some_and(|current| {
            ...
            self.ready = false;
            return Err(ActorGroupStateError::StaleTerm);
        }
        let mut records = BTreeMap::new();
        for record in snapshot.records {
            if records.insert(record.key, record.value).is_some() {
                return Err(ActorGroupStateError::DuplicateRecord);
            }
        }
        self.version = Some(version);
        self.records = records;
    ...
    pub fn apply(&mut self, delta: CoordinatorDelta) -> Result<(), ActorGroupStateError> {
        if delta.version.group != self.group {
            return Err(ActorGroupStateError::GroupMismatch);
        }
        let current = self
            .version
            .as_ref()
            .ok_or(ActorGroupStateError::SnapshotRequired)?;
        if !current.accepts_delta_after(&delta.version) {
            self.ready = false;
            return if delta.version.term > current.term {
                Err(ActorGroupStateError::SnapshotRequired)
            } else if delta.version.term < current.term {
                Err(ActorGroupStateError::StaleTerm)
            } else {
...
#[derive(Debug, Clone, PartialEq, Eq, Error)]
pub enum ActorGroupStateError {
    #[error("actor-group snapshot is required")]
    SnapshotRequired,
    #[error("actor-group revision gap requires a snapshot")]
    RevisionGap,
    #[error("actor-group state belongs to a stale term")]
    StaleTerm,
    #[error("actor-group snapshot contains a duplicate key")]
    DuplicateRecord,
    #[error("placement snapshot belongs to another group")]
    GroupMismatch,
    #[error("membership snapshot cannot be installed in actor-group state")]
    UnexpectedSnapshot,
}
...
pub enum CoordinatorError {
    ...
    InvalidMember,
    #[error("actor-group member record is invalid")]
    InvalidGroupMember,
