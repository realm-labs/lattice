...
#[test]
fn singleton_config_fingerprint_has_a_group_scoped_golden_vector() {
    let config = SingletonConfig::new(
        ActorGroupId::new("battle").unwrap(),
...
#[test]
fn placement_group_hello_builder_preserves_defaults_and_configuration() {
    let group = ActorGroupId::new("battle").unwrap();
    let node = NodeKey {
        node_id: "node-a".to_owned(),
        address: NodeEndpoint::new("127.0.0.1", 25520).unwrap(),
        incarnation: NodeIncarnation::new(1).unwrap(),
    };
    let empty = ActorGroupHello::builder(node.clone(), group.clone(), 1).build();
    assert!(empty.hosted_entity_types.is_empty());
    assert!(empty.constraints.is_empty());
    assert_eq!(
        empty.group_config_fingerprint,
        placement_group_fingerprint(&group)
    );
    ...
    let configured = ActorGroupHello::builder(node, group, 8)
...
fn higher_term_delta_requires_snapshot_and_lower_term_snapshot_is_stale() {
    let mut session = ActorGroupState::new(ActorGroupId::new("test").unwrap());
    session
    ...
    assert_eq!(
        session
            ...
            .unwrap_err(),
        ActorGroupStateError::SnapshotRequired
    );
    ...
    assert_eq!(
        session
            ...
            .unwrap_err(),
        ActorGroupStateError::StaleTerm
    );
    assert!(!session.ready());
}
