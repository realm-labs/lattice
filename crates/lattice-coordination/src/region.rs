...
use lattice_model::cluster::{
    ActorGroupId, ClusterId, ConfigFingerprint, EntityType, NodeIncarnation,
};
use thiserror::Error;
...
impl EntityConfig {
        ...
        P: Into<String>,
    {
        ...
        if shard_mapper_id == XXH3_V1_MAPPER_ID && shard_mapper_version == XXH3_V1_MAPPER_VERSION {
            // The built-in mapper's fixed seed is part of its configuration identity.
...
mod tests {
    ...
    fn entity() -> EntityConfig {
        EntityConfig::new(
            ActorGroupId::new("buffer-test").unwrap(),
    ...
    #[test]
    fn entity_config_fingerprint_has_a_group_scoped_golden_vector() {
        ...
        let another_group = EntityConfig::new(
        ...
        .unwrap();
        assert_ne!(config.fingerprint(), another_group.fingerprint());
    }

    #[test]
    fn mapper_identity_is_explicit_in_the_configuration() {
        let config = entity();
        let encoded = serde_json::to_value(&config).unwrap();
        assert_eq!(encoded["shard_mapper_version"], XXH3_V1_MAPPER_VERSION);
        assert_eq!(encoded["shard_mapper_id"], XXH3_V1_MAPPER_ID);
        assert!(encoded.get("shard_hash_version").is_none());
        let mut missing_mapper = encoded.clone();
        missing_mapper
            .as_object_mut()
            .unwrap()
            .remove("shard_mapper_id");
        assert!(serde_json::from_value::<EntityConfig>(missing_mapper).is_err());
    ...
    fn a_full_home_cache_evicts_the_coldest_route_instead_of_rejecting_the_update() {
        ...
        let remote = NodeIncarnation::new(2).unwrap();
        let config = EntityConfig::new(
            ActorGroupId::new("home-cache").unwrap(),
