...
use crate::{
    ...
    coordinator::{
        ActorGroupHello, CoordinatorError, GroupLeaderGuard, GroupMemberRecord, GroupMemberStatus,
        LeaderRecord, LoadTable, MemberRecord, MemberRemovalReason, MemberStatus, SessionLimits,
    ...
    storage::{
        ...
        page::PageCursor,
        records::{AdminOperationRecord, AutomaticBalanceSettings, DurableStorageLimits},
    },
    types::{
...
struct MemberSession {
    hello: ActorGroupHello,
    record: MemberRecord,
    group_record: Option<GroupMemberRecord>,
...
impl MemberSession {
    fn placement_up(&self) -> bool {
        self.record.status == MemberStatus::Up
            && self
                .group_record
                .as_ref()
                .is_some_and(|member| member.status == GroupMemberStatus::Up)
    }
    ...
    fn placement_attached(&self) -> bool {
        self.record.status == MemberStatus::Up
            && self.group_record.as_ref().is_some_and(|member| {
                matches!(
                    member.status,
                    GroupMemberStatus::Up | GroupMemberStatus::Leaving
                )
            })
    }
}
...
#[derive(Clone)]
pub struct GroupCoordinatorHandle {
    group: ActorGroupId,
...
impl GroupCoordinatorHandle {
    ...
    pub async fn relocate_shard(
        ...
        self.require_group(&request.group)?;
        let (completion, result) = tokio::sync::oneshot::channel();
        self.operations
    ...
    fn require_group(&self, group: &ActorGroupId) -> Result<(), CoordinatorRuntimeError> {
        if group == &self.group {
            Ok(())
        } else {
    ...
    S: CoordinatorLeaseStore + ScopedElectionStore + MembershipStore + ActorGroupStore,
{
    ...
    pub async fn elect_with_strategies(
        ...
        let group = match &scope {
            ...
            CoordinatorScope::Cluster => {
                return Err(CoordinatorRuntimeError::invalid_config(
                    "a actor-group leader requires a placement scope",
                ));
            }
        };
        ...
        let leader_guard =
            GroupLeaderGuard::new(leader.clone()).map_err(CoordinatorRuntimeError::Coordinator)?;
        ...
        let plans = store
            ...
            .collect();
        let automatic_settings = store.get_automatic_settings(&group).await?;
        let applied_admin_operations = store
            .list_admin_operations(&group)
            .await?
            .into_iter()
    ...
    async fn assignment_members(
        &self,
        owner: &NodeKey,
    ) -> Result<(MemberRecord, GroupMemberRecord), CoordinatorRuntimeError> {
        let global = self
            .store
        ...
        let group = self
            .store
            .get_group_member(&self.version.group, &owner.node_id)
            .await?
            .filter(|member| {
                member.node == *owner
                    && member.status == GroupMemberStatus::Up
                    && member.version.group == self.version.group
            })
            .ok_or(CoordinatorRuntimeError::IneligibleTarget)?;
        Ok((global, group))
    }
    ...
    pub(super) fn observe_slot(&mut self, slot: &PlacementSlot) {
        if slot.key.group() != &self.version.group {
            return;
        }
    ...
    pub fn handle(&self) -> GroupCoordinatorHandle {
        GroupCoordinatorHandle {
            group: self.version.group.clone(),
...
pub enum CoordinatorRuntimeError {
    ...
    UnknownEntityConfig,
    #[error("shard ID is outside the configured entity key group")]
    ShardOutOfRange,
