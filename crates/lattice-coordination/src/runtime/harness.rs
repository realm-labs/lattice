//! A driveable [`GroupCoordinator`] for fault-injection harnesses.
//!
...
use super::{
    CoordinatorRuntimeError, GroupCoordinator, GroupCoordinatorConfig, ManualRelocationRequest,
};
use crate::{
    control::{DEFAULT_MAX_CONTROL_PAYLOAD, PlacementControlCommand, decode_control_command},
    coordinator::{
        ActorGroupHello, ClusterLeaderGuard, LeaderRecord, MemberHello, MemberRecord, MemberStatus,
    },
    ...
    storage::{
        ...
        ScopedElectionStore, records::CreateMember,
    },
    types::{
...
impl GroupHarness {
    pub async fn start(
        ...
        let group_id = ActorGroupId::new(group).map_err(rejected)?;
        ...
        let mut leader = GroupCoordinator::elect(
            ...
            associations.clone(),
            coordinator.clone(),
            CoordinatorScope::Group(group_id.clone()),
        ...
        for (index, node) in nodes.into_iter().enumerate() {
            ...
            let hello = ActorGroupHello::builder(node.clone(), group_id.clone(), 4)
    ...
    pub async fn stored_plans(&self) -> Result<usize, CoordinatorRuntimeError> {
        Ok(self.store.list_plans(&self.group).await?.len())
    }
    ...
    pub async fn stored_automatic_paused(&self) -> Result<bool, CoordinatorRuntimeError> {
        Ok(self
            .store
            .get_automatic_settings(&self.group)
            .await?
    ...
    pub async fn stored_admin_operations(&self) -> Result<usize, CoordinatorRuntimeError> {
        Ok(self.store.list_admin_operations(&self.group).await?.len())
    }
...
async fn ensure_global_member(
    ...
    let guard = ClusterLeaderGuard::new(membership).map_err(rejected)?;
    leader
        .store
