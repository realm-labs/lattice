    ...
    S: CoordinatorLeaseStore + ScopedElectionStore + MembershipStore + ActorGroupStore,
{
    ...
    /// group immediately; the periodic full scan only closes gaps a dropped event would leave.
    ...
    async fn send_membership_snapshot(
        ...
        let (begin, chunks, end) = build_snapshot(
            ...
            version.term.get(),
            config.group.maximum_control_payload,
            SnapshotVersion::Membership(version),
            records,
            &config.group.snapshot_limits,
        )
            ...
            .chain(std::iter::once(PlacementControlCommand::SnapshotEnd(end)))
        {
            ...
            association
                .admit_control_command_in_wait(
                    crate::control::control_stream_id(&CoordinatorScope::Cluster),
                    payload,
                    wait_timeout,
                )
                .await?;
    ...
    pub(super) fn spawn_global_member_reconciliation(&mut self) {
        for (group, hosted) in &self.groups {
            ...
            if !self.reconciling_groups.insert(group.clone()) {
                continue;
            }
            let store = self.store.clone();
            ...
            let maximum_work = self.config.group.maximum_reconciliation_work_per_pass;
            self.background_tasks.spawn(
                HostBackgroundCompletion::DomainReconciliation(completed_group),
                async move {
                    let result = async {
                        let participants = store.list_group_members(&group).await?;
                        for participant in participants.into_iter().take(maximum_work) {
                            ...
                            try_remove_global_member_from_group(
                                &sender,
                                participant.node,
    ...
    pub(super) async fn fanout_global_member_removal(
        ...
        for hosted in self.groups.values() {
            ...
            try_remove_global_member_from_group(sender, node.clone(), reason)?;
        }
        Ok(())
    }
}

fn try_remove_global_member_from_group(
