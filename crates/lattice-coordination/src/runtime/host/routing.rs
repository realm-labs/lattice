    ...
    S: CoordinatorLeaseStore + ScopedElectionStore + MembershipStore + ActorGroupStore,
{
            ...
            PlacementControlEventKind::Command(inbound) => {
                ...
                match (&inbound.scope, &inbound.command) {
                    ...
                    (
                        ...
                        let Some(hosted) = self.groups.get(group) else {
                    ...
                    (CoordinatorScope::Group(group), _) => {
    ...
    pub(super) fn active_term(&self, scope: &CoordinatorScope) -> Option<u64> {
        let state = match scope {
            CoordinatorScope::Cluster => &self.membership_state,
            CoordinatorScope::Group(group) => &self.groups.get(group)?.state,
        };
        match state {
