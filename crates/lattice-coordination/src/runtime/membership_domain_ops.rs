    ...
    S: CoordinatorLeaseStore + ScopedElectionStore + MembershipStore + ActorGroupStore,
{
    pub(super) async fn begin_member_drain(
        ...
        member.status = GroupMemberStatus::Leaving;
        ...
        let session = self
            ...
            .get_mut(&incarnation)
            .ok_or(CoordinatorRuntimeError::UnknownSession)?;
        session.group_record = Some(member.clone());
    ...
    async fn confirm_member_drain(
        ...
        if self
            .store
            .get_group_member(&self.version.group, node_id)
            .await?
    ...
    pub(super) async fn remove_member(
        ...
        if let Some(group_member) = self
            .store
            .get_group_member(&self.version.group, &member.node.node_id)
            .await?
        {
            self.store
                .remove_group_member(
    ...
    pub(super) async fn finish_node_removal(
        ...
        // Removing the departing group member advanced the shared placement
        ...
        let remaining_slots = self.store.list_slots(&self.version.group).await?;
    ...
    async fn remove_global_member_participation(
        ...
        if self
            ...
            .await?
            .is_some_and(|member| member.node == node && member.status == MemberStatus::Up)
        {
            return Ok(());
        }
        let Some(group_member) = self
            .store
            .get_group_member(&self.version.group, &node.node_id)
            .await?
    ...
    pub(super) async fn send_snapshot(
        ...
        for member in self.store.list_group_members(&self.version.group).await? {
            placement_records.push(SnapshotRecord {
                key: format!(
                    "group/{}/member/{}",
                    self.version.group.as_str(),
                    member.node.node_id
                ),
                value: Bytes::from(
        ...
        for slot in self.store.list_slots(&self.version.group).await? {
        ...
        let placement = build_snapshot(
            &CoordinatorScope::Group(self.version.group.clone()),
            self.version.term.get(),
    ...
    /// one joining member cannot monopolize the group loop with an inventory-sized store sweep.
