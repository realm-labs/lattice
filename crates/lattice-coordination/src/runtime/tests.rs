...
fn group() -> ActorGroupId {
    ActorGroupId::new("runtime-test").unwrap()
}
use lattice_remoting::{
...
use crate::{
    ...
    storage::{
        ActorGroupStore, InMemoryCoordinationStore,
        records::{
...
async fn ensure_test_global_member(
        ...
        .is_none()
    {
        ...
        leader
            .store
            .create_member(
                &ClusterLeaderGuard::new(membership_record).unwrap(),
                CreateMember { member },
            )
            .await
            .unwrap();
...
async fn register_up(
    ...
    leader
        .register(hello.group, association.clone())
        .await
        .unwrap();
    leader
...
async fn seed_running_slot(
    ...
    let expected_group_member = if let Some(member) = leader
        .store
        .get_group_member(&leader.version.group, &owner.node_id)
        .await
        .unwrap()
    {
        member
...
fn test_hello(node: NodeKey, spec: TestHelloSpec) -> TestHello {
    TestHello {
        ...
        group: ActorGroupHello::builder(node, group(), spec.capacity_units)
...
#[tokio::test]
async fn joining_group_member_advances_existing_sessions_to_latest_revision() {
    let cluster_id = ClusterId::new("group-join-revision-test").unwrap();
...
async fn real_control_session_installs_snapshot_and_matching_claim() {
    ...
    .await;
    leader
        .register(hello.group.clone(), coordinator_to_logic)
        .await
        .unwrap();
    let (logic, mut effects) = GroupSession::new(
        hello.group,
...
async fn persisted_handoff_barrier_replaces_claim_forward() {
    ...
    let plan = store.get_plan(&group(), plan_id).await.unwrap().unwrap();
...
async fn first_resolution_allocates_shard_and_singleton_to_declared_host() {
    ...
    let singleton_config = SingletonConfig::new(group(), singleton_kind.clone(), protocol_id);
    let descriptor = ProtocolDescriptor {
        protocol_id,
...
async fn resolution_reassigns_fenced_slots_after_owner_restart() {
    ...
    store.insert_slot_record(PlacementSlot {
        key: shard_key.clone(),
