...
async fn logic_session_replacement_and_member_removal_clear_the_bounded_load_entry() {
    ...
    fixture
        .leader
        .register(fixture.host_hello.group.clone(), fixture.host_key.clone())
        .await
        .unwrap();
    assert!(
        fixture
...
async fn allocating_resolution_publishes_a_slot_delta_without_a_full_snapshot() {
    ...
    assert!(
        ...
        "resolution must not re-encode the whole group snapshot"
    );
...
async fn a_member_reaching_the_leader_revision_defers_missing_claims_to_bounded_reconciliation() {
    ...
    assert_eq!(fixture.proxy_hello.group.node, fixture.proxy);
}
...
async fn singleton_kinds_spread_across_one_eligibility_set() {
    ...
    let configs = kinds
        .iter()
        .map(|kind| SingletonConfig::new(group(), kind.clone(), protocol_id))
        .collect::<Vec<_>>();
...
async fn member_removal_survives_a_surviving_session_whose_association_is_gone() {
    ...
    fixture
        ...
        .await
        .expect("an unreachable surviving session must not end the group leader");
