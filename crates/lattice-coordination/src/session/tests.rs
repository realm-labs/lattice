...
async fn admission_closes_on_the_installed_deadline_even_though_no_tick_ran() {
    let group = ActorGroupId::new("frozen-owner").unwrap();
    let local = NodeKey {
...
async fn effect_backpressure_waits_for_capacity_without_terminating_the_session() {
    ...
    let group = ActorGroupId::new("effect-backpressure").unwrap();
    let local = NodeKey {
    ...
    let (session, mut effects) = GroupSession::new(
        ActorGroupHello::builder(local, group.clone(), 1).build(),
        coordinator,
        associations,
...
async fn control_queue_backpressure_does_not_terminate_the_session() {
    ...
    let group = ActorGroupId::new("control-admission-backpressure").unwrap();
    let local = NodeKey {
        node_id: "logic".to_owned(),
    ...
    let (session, _effects) = GroupSession::new(
        ActorGroupHello::builder(local, group, 1).build(),
        coordinator,
        associations,
        GroupSessionConfig::default(),
        8,
...
async fn reliable_outbox_backpressure_does_not_terminate_the_session() {
    ...
    let group = ActorGroupId::new("control-outbox-backpressure").unwrap();
    let local = NodeKey {
        node_id: "logic".to_owned(),
...
async fn runtime_progress_never_consumes_the_reliable_control_outbox() {
    ...
    let group = ActorGroupId::new("runtime-progress-outbox").unwrap();
    let local = NodeKey {
        node_id: "logic".to_owned(),
    ...
    let (session, _effects) = GroupSession::new(
        ActorGroupHello::builder(local.clone(), group.clone(), 1).build(),
        coordinator,
        associations,
        GroupSessionConfig::default(),
        8,
...
async fn an_unacknowledged_drain_completion_gives_up_instead_of_polling_forever() {
    ...
    let (session, _effects) = GroupSession::new(
        ActorGroupHello::builder(local, ActorGroupId::new("drain-timeout").unwrap(), 1).build(),
        coordinator,
        associations,
        config.clone(),
        8,
...
async fn heartbeat_publishes_a_fresh_baseline_node_load_sample() {
    ...
    let (session, _effects) = GroupSession::new(
        ActorGroupHello::builder(
            local.clone(),
            ActorGroupId::new("automatic-node-load").unwrap(),
            1,
        )
        .build(),
        coordinator.clone(),
    ...
    while let Ok(frame) = outbound.try_recv() {
        ...
        if matches!(scoped.command, PlacementControlCommand::ActorGroupHello(_)) {
            hello_count += 1;
        }
    }
    assert!(
        hello_count >= 2,
        "group hello should be retried before registration"
    );
    ...
    controls
        .send(PlacementControlEvent {
            kind: PlacementControlEventKind::Command(Box::new(InboundPlacementControl {
                association: coordinator,
                command_id: CommandId::generate(),
                scope: CoordinatorScope::Group(ActorGroupId::new("automatic-node-load").unwrap()),
                coordinator_term: Some(1),
