...
use super::{
    InMemoryCoordinationStore, StorageError,
...
impl InMemoryCoordinationStore {
    ...
    pub(super) async fn list_plans_page_inner(
        ...
        let start = position::<(ActorGroupId, u128)>(cursor)?;
        let state = self.inner.lock().expect("placement memory store poisoned");
        let scan = paginate(
            state.plans.range((start, Bound::Unbounded)),
            limit,
            |(candidate, _), _| candidate == group,
            |_| true,
            Clone::clone,
        )?;
        drop(state);
    ...
    pub(super) async fn list_claims_page_inner(
        ...
        let scan = paginate(
            ...
            limit,
            |_, claim: &LeasedClaim| &claim.grant.group == group,
            |_| true,
            Clone::clone,
        )?;
        drop(state);
