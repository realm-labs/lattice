...
impl PlacementVersion {
    pub fn new(group: ActorGroupId, term: CoordinatorTerm, revision: Revision) -> Self {
        Self {
            group,
            term,
            revision,
...
impl PlacementSlotKey {
    pub fn group(&self) -> &ActorGroupId {
        match self {
            Self::Shard { group, .. } | Self::Singleton { group, .. } => group,
        }
    }
}
...
impl ClaimGrant {
    pub fn validate(&self, safety_margin: Duration) -> Result<(), PlacementTypeError> {
        self.owner.validate()?;
        if &self.group != self.slot.group() {
            return Err(PlacementTypeError::ClaimGroupMismatch);
        }
...
pub enum PlacementTypeError {
    ...
    InvalidClaimTtl,
    #[error("claim group does not match its placement slot group")]
    ClaimGroupMismatch,
}
