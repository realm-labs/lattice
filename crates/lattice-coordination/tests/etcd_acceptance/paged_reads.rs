...
use lattice_coordination::storage::page::PageCursor;
...
async fn real_etcd_slot_pages_are_bounded_ranges_over_both_slot_prefixes() {
    ...
    let filtered = store
        .list_slots_page(&group(), &[PlacementSlotState::Fenced], None, 3)
        .await
        .unwrap();
...
async fn cursor_fixture(
    ...
    name: &str,
    shards: &[u32],
) -> (EtcdCoordinationStore, Client, String) {
...
async fn real_etcd_removing_a_record_behind_the_cursor_skips_no_later_record() {
    ...
    let first = store.list_slots_page(&group(), &[], None, 2).await.unwrap();
    assert_eq!(
