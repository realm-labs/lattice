...
use crate::session::LogicSessionError;
...
impl DrainConfirmation {
    pub(crate) fn request(&self, operation_id: &str, term: u64) -> Result<(), LogicSessionError> {
        ...
        self.0.send_modify(|state| {
            if state.closed {
                result = Err(LogicSessionError::ControlClosed);
            } else if state
                .request
                .as_ref()
                .is_some_and(|(current, _)| current != operation_id)
            {
                result = Err(LogicSessionError::InvalidConfig);
            } else if state
                .request
                .as_ref()
    ...
    pub(crate) fn confirm(&self, operation_id: &str, term: u64) -> Result<(), LogicSessionError> {
