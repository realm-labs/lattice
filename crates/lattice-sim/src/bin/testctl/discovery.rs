...
pub fn verify(artifacts: &Path) -> Result<(), String> {
    let cases = [
        ("discovery-static.json", "static"),
        ("discovery-config.json", "config-store"),
    ];
    for (name, provider) in cases {
        ...
        std::fs::write(path.with_extension("leave"), b"leave")
