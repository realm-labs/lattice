...
Service assembly and the placement view installed in `ServiceContext` depend on the read/watch-only `PlacementReadStore`. Actor/singleton routing, singleton claim discovery, and admin inspection have narrower independent capabilities and builder slots. `TonicPlacementRoutingStore` has only semantic point/watch access. `TonicPlacementReader` derives exact-boot singleton claims from the bounded ownership snapshot. Its admin reader combines that snapshot with backend-bounded service-instance enumeration; real etcd applies `limit = max + 1` before response decoding, and overflow is terminal. Admin drain target lookup also uses the semantic point path. Assembly tests prove the routing, claim, and admin slots are selected; real mTLS tests cover their successful current-boot behavior, capacity failure, and stale denial. This remains an incomplete deployment boundary: all slots are optional and no bundled production bootstrap selects them; the public broad read view and ownership lifecycle retain the concrete etcd connection and its raw Lease protocol capability. Every consumer must be cut over and that credential removed before reclamation authority can be issued.
