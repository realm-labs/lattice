...
Service assembly and the placement view installed in `ServiceContext` depend on the read/watch-only `PlacementReadStore`. Actor and singleton route resolvers instead depend on `PlacementRoutingStore`, a deliberately narrower capability containing only instance/owner point reads and a service-scoped watch. `TonicPlacementRoutingStore` satisfies it through `TonicPlacementReader` and has no backend or list access; its bounded cancellable bridge maps proof-validated ownership updates and deletes into cache invalidations. Real mTLS coverage exercises resolution and later watch delivery through this adapter. The authenticated coordinator admits the current boot before each lookup or stream creation, and one backend `OwnershipView` supplies both the first coherent snapshot frame and all subsequent progress/same-revision frames. The public service view exposes no lease grant/keepalive, instance upsert, activation, drain, compare-and-put, or lock methods. The private erased store handle likewise exposes no lease mutation, while liveness and singleton renewal cross `PlacementAuthority`. This remains an incomplete deployment boundary: service assembly does not yet select the semantic routing adapter, and admin reads, singleton claim discovery, and ownership lifecycle retain the concrete etcd read/watch connection and its raw Lease protocol capability. Every consumer must be cut over and that credential removed before reclamation authority can be issued.
