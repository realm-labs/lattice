...
Service assembly and the placement view installed in `ServiceContext` depend on the read/watch-only `PlacementReadStore`. Actor/singleton routing, singleton claim discovery, admin inspection, and local ownership lifecycle have narrower independent capabilities and builder slots. `OwnershipViewReader` can be backed by `TonicPlacementReader`; after the exact boot is published Ready, the service installs its bounded coherent snapshot before publishing its readiness signal, applies ordered watch batches to `LocalOwnershipSnapshot`, and fences the gate on stream/proof or keepalive failure. Assembly/lifecycle tests prove a later hardened commit reaches the gate through that watch; real mTLS tests cover the semantic stream's current-boot admission, capacity failure, and stale denial. This remains an incomplete deployment boundary: all slots are optional, generated ingress does not yet use the gate, no bundled production bootstrap selects them, and the public broad read view retains the concrete etcd connection and its raw Lease protocol capability. Every consumer must be cut over and that credential removed before reclamation authority can be issued.
