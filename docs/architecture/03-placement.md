...
Service assembly and the placement view installed in `ServiceContext` depend on the read/watch-only `PlacementReadStore`. Actor and singleton route resolvers instead depend on `PlacementRoutingStore`, a deliberately narrower capability containing only instance/owner point reads and a service-scoped watch. `TonicPlacementRoutingStore` satisfies it through `TonicPlacementReader` and has no backend or list access; its bounded cancellable bridge maps proof-validated ownership updates and deletes into cache invalidations. `LatticeServiceBuilder::placement_routing_store` installs this capability independently, and generated actor/singleton client resolvers prefer it over the broad store. Assembly coverage proves the separate watch is selected; real mTLS coverage exercises both resolver families and later watch delivery. The authenticated coordinator admits the current boot before each lookup or stream creation, and one backend `OwnershipView` supplies both the first coherent snapshot frame and all subsequent progress/same-revision frames. This remains an incomplete deployment boundary: the routing slot is optional and no bundled production bootstrap selects it; admin reads, singleton claim discovery, the public broad read view, and ownership lifecycle retain the concrete etcd connection and its raw Lease protocol capability. Every consumer must be cut over and that credential removed before reclamation authority can be issued.
