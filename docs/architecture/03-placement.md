...
The placement view installed in `ServiceContext` is read/watch-only. It exposes no lease grant/keepalive, instance upsert, activation, drain, compare-and-put, or lock methods; compile-fail tests cover the public boundary. The private erased store handle likewise exposes no instance registration, renewal, or state mutation: service instance lifecycle crosses `PlacementAuthority`. It still exposes a dedicated singleton-owner renewal operation, and its concrete etcd read/watch connection retains raw Lease protocol capability. Singleton cutover and then removal of the direct etcd connection are required before reclamation authority can be issued.
