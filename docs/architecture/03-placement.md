...
Service assembly and the placement view installed in `ServiceContext` depend on the read/watch-only `PlacementReadStore`. Actor and singleton route resolvers instead depend on `PlacementRoutingStore`, a deliberately narrower capability containing only instance/owner point reads and a service-scoped watch. `TonicPlacementRoutingStore` satisfies it through `TonicPlacementReader` and has no backend or list access; `placement_routing_store` makes generated actor/singleton resolvers prefer it. Singleton renewal discovery has a separate `SingletonClaimReader` capability and builder slot. The tonic reader derives claims from the bounded coherent service snapshot and filters exact owner ID plus boot incarnation before semantic renewal; the keepalive loop bypasses broad-store discovery when configured. Assembly tests prove both slots are selected, and real mTLS tests cover routing, watch delivery, current-boot claim discovery, and prior-boot exclusion. This remains an incomplete deployment boundary: both slots are optional and no bundled production bootstrap selects them; admin reads, the public broad read view, and ownership lifecycle retain the concrete etcd connection and its raw Lease protocol capability. Every consumer must be cut over and that credential removed before reclamation authority can be issued.
