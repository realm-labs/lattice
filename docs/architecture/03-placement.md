...
The semantic authority surface currently contains `ActivateActor`, `ActivateSingleton`, and `DrainInstance`. After tonic's bounded protobuf decode, the production service authenticates the verified leaf certificate before inspecting request fields or entering any placement mutation. It accepts exactly one canonical URI SAN of the form `spiffe://<trust-domain>/svc/<service>/instance/<instance>/incarnation/<incarnation>`; caller metadata and manually installed `PeerIdentity` extensions are not authentication. Every method then performs an exact service/instance lookup and admits only a certificate incarnation matching the current `Ready`, nonzero-leased `InstanceRecord`. Thus a still-valid certificate from an earlier boot cannot activate or drain after the instance ID is reused. Actor and singleton activation remain available to current authenticated workloads in the configured trust domain because cross-service resolution is expected. Drain is narrower: a workload may drain only the exact service, instance, and incarnation named by its certificate, and the expected current lease must still match atomically. A separate operator/admin principal is not implemented, so remote peer-instance drain is deliberately denied.

Authority calls have a five-second default deadline and reject zero or greater-than-sixty-second configured deadlines. Replies are accepted only when every requested identity is echoed exactly, owner/epoch/lease fields are valid, activation state is `Running`, and drain counts fit the local platform. Normal shutdown drain carries the runtime's expected instance incarnation and lease; the authority atomically changes state only if service, instance, incarnation, and lease still match, so a stale process cannot drain a replacement that reused its instance ID. The same four-part backend CAS guards `Ready` and `Stopping`; incarnation is independent of the numeric lease and still fences if a broken or restored backend reuses that lease value. The service performs no blind pre-drain state write. Admin self-drain first reads the current incarnation and lease and submits the same fenced operation. Missing targets are RPC errors, while a valid drain with no replacement is an explicit identity-, incarnation-, lease-, and count-validated successful-reply outcome. Generic transport or proxy `RESOURCE_EXHAUSTED` errors remain failures and are never interpreted as graceful shutdown. Explicit-actor route resolution then additionally requires a `Ready` owner whose instance lease exactly matches the placement record. Direct Link target resolution enforces the same state and lease lineage. Singleton records intentionally use a separate owner lease, so singleton routing requires a `Running` record and `Ready` instance; the later dedicated singleton lifecycle work must prove renewal, expiry, and reincarnation rather than equating that lease with the instance lease.

This API boundary is necessary but not yet a complete reclamation boundary. Real tonic coverage proves accepted current-incarnation mTLS identity plus rejection of missing certificates, untrusted issuers, wrong server trust/host, plaintext, malformed or wrong-domain SPIFFE IDs, a different workload spoofing a drain target, and a stale certificate for the same reusable service/instance across all methods without mutating placement. Certificate reload/revocation is not implemented, coordinator-to-runtime `LogicControl` remains plaintext, and runtime instance/singleton liveness still uses direct etcd lease operations. Until liveness is isolated behind incarnation-bound semantic authority and the remaining writer-cutover work is complete, authenticated authority RPC is not sufficient evidence for tombstone reclamation.
