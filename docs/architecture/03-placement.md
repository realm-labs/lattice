...
Service assembly, placement route resolvers, and the placement view installed in `ServiceContext` depend on the read/watch-only `PlacementReadStore`. `ReadOnlyPlacementStore` deliberately does not implement the writable `PlacementStore`, and executable lifecycle coverage runs a service through that restricted capability while a separate authority owns mutation. The authenticated coordinator exposes bounded semantic point reads plus a no-gap per-service ownership stream through `TonicPlacementReader`; the same current-ready boot-incarnation admission used for mutation runs before each backend lookup or stream creation. One backend `OwnershipView` supplies both the first coherent snapshot frame and all later progress/same-revision batch frames, avoiding a second subscribe race. A bounded 16-frame bridge preserves backpressure, while lag, capacity, startup backlog, compaction, cancellation, protocol, and proof failures are terminal structured frames. Each placement record carries context, record/floor versions, and floor epoch; the client reconstructs the opaque snapshot/upsert/delete proof through the lineage constructor and returns the native cancellable `OwnershipView`. The public view exposes no lease grant/keepalive, instance upsert, activation, drain, compare-and-put, or lock methods; compile-fail tests cover that boundary. The private erased store handle likewise exposes no lease mutation: it only discovers a bounded set of current-boot singleton records, while instance and singleton renewal cross `PlacementAuthority`. This remains an incomplete deployment boundary: service assembly, routing watches, admin reads, and singleton claim discovery still retain the concrete etcd read/watch connection and its raw Lease protocol capability. Every consumer must be cut over and that credential removed before reclamation authority can be issued.
