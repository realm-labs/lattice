...
Service assembly, placement route resolvers, and the placement view installed in `ServiceContext` depend on the read/watch-only `PlacementReadStore`. `ReadOnlyPlacementStore` deliberately does not implement the writable `PlacementStore`, and executable lifecycle coverage runs a service through that restricted capability while a separate authority owns mutation. The authenticated coordinator exposes bounded semantic instance, actor, and singleton point reads plus a coherent per-service snapshot through `TonicPlacementReader`; the same current-ready boot-incarnation admission used for mutation runs before each backend lookup. The snapshot reuses the backend's floor-proven ownership-view transaction and bounds the combined placement set before publication, while the client enforces its own cardinality, identity, uniqueness, revision, and record validation under a deadline. The public view exposes no lease grant/keepalive, instance upsert, activation, drain, compare-and-put, or lock methods; compile-fail tests cover that boundary. The private erased store handle likewise exposes no lease mutation: it only discovers a bounded set of current-boot singleton records, while instance and singleton renewal cross `PlacementAuthority`. This remains an incomplete network boundary: the snapshot omits floor proofs needed for local authorization, no-gap watch streaming is absent, and service assembly is not cut over, so the wrapper's concrete etcd read/watch connection retains raw Lease protocol capability. Removing that direct connection through the completed read/watch proxy is required before reclamation authority can be issued.
