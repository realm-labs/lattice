...
Both `placement_store` and `placement_authority` are mandatory service components; the builder has no implicit in-memory placement fallback. The former accepts only the `PlacementReadStore` capability, while `TonicPlacementAuthority` accepts a caller-built channel, applies a bounded per-call deadline, and exposes instance registration/renewal/transition, bounded singleton renewal, actor and singleton activation, and drain. `TonicPlacementReader` uses that authenticated channel for bounded instance, actor, and singleton point reads and a coherent, backend-bounded per-service snapshot without retaining a backend client. A production caller must build the channel with the coordinator CA, the expected server DNS name, and its boot-incarnation-bound client certificate/key; the standalone server requires that verified SPIFFE identity to match the current leased instance before each read or mutation. A plaintext, stale-certificate, or otherwise misconfigured caller cannot bypass an mTLS-configured server's admission, though the supplied-channel clients report that failure on their first call rather than validating channel construction itself. Runtime list and watch operations still use a direct etcd connection; the semantic snapshot is not yet wired and omits floor proofs, and removing unrestricted Lease RPC access still requires no-gap watch RPCs plus service cutover before reclamation can be enabled.
