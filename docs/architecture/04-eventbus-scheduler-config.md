...
`placement_store` and `placement_authority` remain mandatory service components; the builder has no implicit production fallback. `placement_routing_store` is an independent optional component. When present, generated actor and singleton clients use it for point resolution and route-cache watching instead of `placement_store`. `TonicPlacementAuthority` uses a caller-built authenticated channel for mutations; `TonicPlacementReader` uses it for bounded reads and a proof-carrying no-gap ownership stream; and `TonicPlacementRoutingStore` exposes only the resolver/cache subset. Production bootstrap must construct these from the coordinator CA, expected server DNS name, and boot-incarnation-bound client certificate/key. The repository does not yet bundle that bootstrap or require the routing slot, and admin reads, singleton claim discovery, the broad ServiceContext view, and ownership lifecycle still retain the direct etcd connection. Those paths must be cut over before the runtime credential can be removed.
