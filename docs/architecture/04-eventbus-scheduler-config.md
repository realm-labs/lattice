...
Both `placement_store` and `placement_authority` are mandatory service components; the builder has no implicit in-memory placement fallback. `TonicPlacementAuthority` accepts a caller-built channel, applies a bounded per-call deadline, and exposes instance registration/renewal/transition, bounded singleton renewal, actor and singleton activation, and drain. A production caller must build that channel with the coordinator CA, the expected server DNS name, and its boot-incarnation-bound client certificate/key; the standalone server requires that verified SPIFFE identity to match the requested registration or current leased instance before mutation. A plaintext, stale-certificate, or otherwise misconfigured caller cannot bypass an mTLS-configured server's admission, though the supplied-channel client reports that failure on its first call rather than validating channel construction itself. Runtime placement reads still use a direct etcd connection; removing its unrestricted Lease RPC access requires the later semantic-proxy cutover before reclamation can be enabled.
