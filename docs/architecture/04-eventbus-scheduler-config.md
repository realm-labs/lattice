...
Both `placement_store` and `placement_authority` are mandatory service components; the builder has no implicit in-memory placement fallback. The former accepts only the `PlacementReadStore` capability, while `TonicPlacementAuthority` accepts a caller-built channel, applies a bounded per-call deadline, and exposes instance registration/renewal/transition, bounded singleton renewal, actor and singleton activation, and drain. `TonicPlacementReader` uses that authenticated channel for bounded point reads and a proof-carrying no-gap ownership stream without retaining a backend client. One RPC returns the initial coherent snapshot first and then ordered watch batches; the client returns the native `OwnershipView`. A production caller must build the channel with the coordinator CA, expected server DNS name, and boot-incarnation-bound client certificate/key; the standalone server requires that verified SPIFFE identity to match the current leased instance before each read, stream creation, or mutation. Runtime consumers are not yet wired to this reader and still use a direct etcd connection. Removing unrestricted Lease RPC access requires cutting over route-cache watching, admin reads, singleton claim discovery, and service ownership lifecycle, then removing the direct runtime credential.
