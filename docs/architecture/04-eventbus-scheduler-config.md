...
`placement_store` and `placement_authority` remain mandatory service components; the builder has no implicit production fallback. `placement_routing_store`, `singleton_claim_reader`, and `admin_placement_reader` are independent optional components. They move generated routing/cache reads, exact-boot renewal discovery, and admin snapshot/drain lookup onto `TonicPlacementReader` and its narrow routing adapter. Admin service-instance enumeration is backend-bounded rather than truncated after an unbounded read. Production bootstrap must construct these from the coordinator CA, expected server DNS name, and boot-incarnation-bound client certificate/key. The repository does not yet require the slots, and the broad ServiceContext view plus ownership lifecycle still retain the direct etcd connection. Those paths must be cut over before the runtime credential can be removed.
