...
Both `placement_store` and `placement_authority` are mandatory service components; the builder has no implicit in-memory placement fallback. The former accepts only `PlacementReadStore`, while actor/singleton resolvers accept the narrower `PlacementRoutingStore`. `TonicPlacementAuthority` uses a caller-built authenticated channel for mutations; `TonicPlacementReader` uses it for bounded point reads and a proof-carrying no-gap ownership stream; and `TonicPlacementRoutingStore` adapts those operations to resolver/cache needs without list or backend access. A production caller must build the channel with the coordinator CA, expected server DNS name, and boot-incarnation-bound client certificate/key. Service assembly does not yet expose a separate routing-store slot, so production consumers are not cut over and still retain the direct etcd connection. Removing unrestricted Lease RPC access requires selecting the semantic adapter in assembly, cutting over admin reads, singleton claim discovery, and ownership lifecycle, then removing the direct runtime credential.
