...
`placement_store` and `placement_authority` remain mandatory service components; the builder has no implicit production fallback. `placement_routing_store` and `singleton_claim_reader` are independent optional components. The former supplies generated actor/singleton resolution and cache watches; the latter supplies exact-boot claims to the renewal loop. `TonicPlacementReader` implements bounded semantic claim discovery from its proof-carrying snapshot, and `TonicPlacementRoutingStore` exposes only the resolver subset. Production bootstrap must construct these from the coordinator CA, expected server DNS name, and boot-incarnation-bound client certificate/key. The repository does not yet require either slot, and admin reads, the broad ServiceContext view, and ownership lifecycle still retain the direct etcd connection. Those paths must be cut over before the runtime credential can be removed.
