...
Production service processes use a least-privilege runtime placement credential for reads/watches and the still-direct exact-instance liveness key. Their connection also retains direct etcd `LeaseGrant`/`LeaseKeepAlive` protocol access for instance and singleton lease renewal until the semantic liveness proxy is complete. They do not receive the placement-authority credential. Password-file authentication keeps secret bytes out of `BootstrapConfig` and command lines:
...
Both `placement_store` and `placement_authority` are mandatory service components; the builder has no implicit in-memory placement fallback. `TonicPlacementAuthority` accepts a caller-built channel, applies a bounded per-call deadline, and exposes only actor activation, singleton activation, and drain. A production caller must build that channel with the coordinator CA, the expected server DNS name, and its boot-incarnation-bound client certificate/key; the standalone server requires the resulting verified SPIFFE identity to match the current `Ready` leased instance record before mutation. A plaintext, stale-certificate, or otherwise misconfigured caller cannot bypass an mTLS-configured server's admission, though the supplied-channel client reports that failure on its first call rather than validating channel construction itself. Runtime placement reads and lease liveness still share a direct etcd connection; removing unrestricted Lease RPC access requires the later semantic-proxy cutover before reclamation can be enabled.
