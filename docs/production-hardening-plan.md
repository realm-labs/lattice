...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven API/backend isolation: raw etcd transport and floor codecs are crate-private with compile-fail coverage; authenticated etcd configuration is bounded and redacted; disposable real-etcd RBAC tests deny runtime actor/floor mutation and prove credential revocation. Service assembly and the public ServiceContext placement view depend on the read/watch-only `PlacementReadStore`; actor and singleton resolvers require the narrower `PlacementRoutingStore`, which exposes only owner/instance point reads and a service-scoped routing watch. `TonicPlacementRoutingStore` implements that contract exclusively through authenticated semantic point reads and the no-gap ownership stream, without list or backend methods. The builder accepts an independent `placement_routing_store`; when configured, every generated actor and singleton client resolver and route-cache watch uses it rather than the broad placement store. Singleton renewal discovery is likewise split behind `SingletonClaimReader`: `TonicPlacementReader` implements it from the bounded proof-carrying service snapshot and filters by exact owner instance and boot incarnation, while `singleton_claim_reader` assembly wiring makes the keepalive loop prefer it over the broad store scan. Executable service coverage proves both independent components are selected; real mTLS coverage proves actor/singleton routing, watch delivery, exact current-boot claim discovery, and prior-boot exclusion through the remote reader. The coordinator stream opens one backend ownership view, sends its coherent initial snapshot first, and forwards ordered progress and same-revision batches through bounded backpressure; lag, capacity, startup backlog, compaction revisions, cancellation, protocol, and proof failures terminate through structured bounded failure frames. Production mutations cross `PlacementAuthority`; create-only registration and lifecycle CAS are boot/lease fenced, and singleton renewal exact-validates at most 4,096 complete claims before renewing any lease. The service's erased broad-store handle exposes no lease mutation. Only the named development adapter restores an in-process authority.
  ...
  - Reclamation remains disabled. Semantic routing and singleton claim-reader assembly slots now exist, but both are optional and no bundled production bootstrap requires them. Admin reads, the broad ServiceContext placement view, and ownership lifecycle remain on the broad store. The concrete runtime read/watch etcd connection therefore still possesses protocol-level LeaseGrant/LeaseKeepAlive capability and could bypass the Rust surface through another client. A partial proxy is insufficient; every production read consumer must use it and the direct credential must be removed or denied Lease RPC at the network/L7 boundary. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
