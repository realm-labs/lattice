...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it. `LocalOwnershipSnapshot` currently retains every tombstone across resync and intentionally fails closed at capacity; do not evict one merely because its placement record is absent.
...
- [ ] Define and test the unsafe placement upgrade preflight: stop old writers; reject startup or clean/migrate slash-bearing keys; CAS-backfill a durable floor from every live actor, virtual-shard, and singleton record before its record can disappear; and seed or prohibit reuse of already-deleted historical identities whose last epoch cannot be reconstructed. Do not claim rolling compatibility with ambiguous keys or writers that do not maintain floors.
