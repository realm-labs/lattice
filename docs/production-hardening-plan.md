...
- [ ] Persist and CAS monotonic per-key epoch floors independently of placement-record leases across deletion and service-process restart for actors, virtual shards, and singletons. Missing-record activation must allocate above the durable floor rather than reset to `Epoch(1)`, and local tombstones may be reclaimed only when the durable floor still prevents stale-route resurrection.
- [ ] Bind singleton placement records to the selected owner's supervised instance lease, or supervise a dedicated placement lease for its full lifetime. Do not leave coordinator-granted singleton leases without a keepalive owner; owner loss, re-registration, deletion, and failover must preserve monotonic epochs.
