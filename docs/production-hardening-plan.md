...
- [x] Persist and CAS monotonic per-key epoch floors independently of placement-record leases across deletion and service-process restart for actors, virtual shards, and singletons created by hardened writers. Non-leased `/authority/epoch_floors/v1` records are reserved before activation and committed atomically with the owner record; opaque `mod_revision` tokens prevent delete/recreate ABA, failed work burns rather than reuses epochs, raw compare-and-put paths cannot bypass the floor, and actor/singleton lock guards are rechecked at commit. Memory, deterministic etcd, chaos, coordinator, and real-etcd coverage exercise all three placement families, including leased singleton deletion, store reconstruction, concurrent reservations, guard loss, overflow, and post-recreation stale tokens. Upgrade backfill is not implemented and remains part of the unchecked preflight item below.
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.

  - Proven foundations: hardened actor, virtual-shard, and singleton writers reject missing or inconsistent floor lineage; snapshots and watch events require exact revision-bound floor proofs; compaction, malformed/miskeyed/leased floors, duplicate selected keys, proof mismatch, and capacity overflow fail closed without partial publication. Singleton records now require the selected owner's boot incarnation in addition to owner ID, epoch, state, and dedicated lease; legacy records without it fail decoding, same-owner reincarnation is an authority change, routing and the local gate reject a replacement boot, and direct renewal ignores prior-boot records. Local state distinguishes present, lifecycle-fenced, exact-delete absent, and coherent-snapshot absent observations, and every absence still consumes capacity.
  - Proven API/backend isolation: raw etcd transport and floor codecs are crate-private with compile-fail coverage; authenticated etcd configuration is bounded and redacted; disposable real-etcd RBAC tests deny runtime actor/floor mutation and prove credential revocation. Service assembly and the public ServiceContext placement view depend on the read/watch-only `PlacementReadStore`; actor and singleton resolvers require the narrower `PlacementRoutingStore`, which exposes only owner/instance point reads and a service-scoped routing watch. `TonicPlacementRoutingStore` implements that contract exclusively through authenticated semantic point reads and the no-gap ownership stream, without list or backend methods. The builder now accepts an independent `placement_routing_store`; when configured, every generated actor and singleton client resolver and route-cache watch uses it rather than the broad placement store. Executable assembly coverage proves the separate watch is selected, while real mTLS coverage resolves actor/singleton targets and observes later placement through the remote adapter. The coordinator stream opens one backend ownership view, sends its coherent initial snapshot first, and forwards ordered progress and same-revision batches through bounded backpressure; lag, capacity, startup backlog, compaction revisions, cancellation, protocol, and proof failures terminate through structured bounded failure frames. Snapshot and watch records carry context-specific floor proofs, and the client reconstructs opaque snapshot/upsert/delete proofs only through the lineage-checking constructor. Production actor/singleton activation, instance registration/renewal/state transition, bounded singleton-owner renewal, and drain cross `PlacementAuthority`; create-only registration allocates its lease inside the backend, rejects an occupied reusable instance key, and revokes a losing etcd lease. Backend CAS also rejects lifecycle regression outside `Starting -> Ready -> Draining/Stopping -> Dead` (with idempotent repeats). Singleton renewal accepts at most 4,096 boot-bound complete-record claims, rejects duplicate keys/leases and stale or changed records before renewing any lease, and returns an identity-checked exact count. The service's erased read-store handle exposes no lease grant/keepalive, instance registration, or state mutation methods. Only the named development adapter restores an in-process authority.
  - Proven authority transport and boot identity: the standalone coordinator now fails closed unless it has complete bounded mTLS material or an explicit loopback-only plaintext escape. In production mTLS mode, every method authenticates exactly one certificate-derived `spiffe://<trust-domain>/svc/<service>/instance/<instance>/incarnation/<incarnation>` identity, then requires an exact keyed lookup to match the current `Ready`, nonzero-leased `InstanceRecord` before semantic request handling or placement mutation. Persisted instance records require a canonical boot incarnation and legacy records without one fail closed. Drain and every service lifecycle state transition require service, instance, incarnation, and lease in the backend CAS; deterministic etcd and service shutdown coverage prove a replacement is not mutated even when a backend reuses the same numeric lease. Real tonic coverage rejects missing/untrusted certificates, wrong server trust/host, plaintext, malformed or wrong-domain identities, spoofed targets, and a valid but stale certificate for the same reusable service/instance across every authority method without advancing the placement revision; it also proves current cross-service activation and exact-incarnation self-drain. The local ownership snapshot is bound to its configured boot incarnation and fences a replacement event even if a backend reuses the lease ID. Generic TLS Debug output redacts certificate and private-key bytes.
  - Reclamation remains disabled. Authenticated semantic point reads, a bounded proof-carrying no-gap ownership stream, a remote-only routing adapter, and an assembly routing slot now exist, but the slot is optional and no bundled production bootstrap selects it. Admin reads, singleton claim discovery, the ServiceContext broad read view, and ownership lifecycle remain on the broad store. The concrete runtime read/watch etcd connection therefore still possesses protocol-level LeaseGrant/LeaseKeepAlive capability and could bypass the Rust surface through another client. A partial proxy is insufficient; every production read consumer must use it and the direct credential must be removed or denied Lease RPC at the network/L7 boundary. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
...
- [ ] Finish and prove the dedicated singleton owner-lease lifecycle. Records and bounded semantic renewal are boot-incarnation-bound; real mTLS and in-memory coverage prove exact current claims renew while duplicate, changed, and prior-boot claims do not. Add real-etcd renewal/expiry/reincarnation coverage, validate keepalive interval versus TTL, stop renewing obsolete records, and clean up newly granted leases when reserve, control, or commit fails while preserving monotonic epochs.
...
- [ ] Gate every inbound Direct Link open and every subsequent message delivery with the durable local ownership snapshot before `ActorRegistry::get_or_load` or handler delivery. A caller can currently connect to the listener directly with `LinkTarget::Endpoint` / a stale direct `ActorRef`; the async open resolver reaches `get_or_load` without an ownership grant, already-open links use `get_running` without revalidation, and a point-read before load would retain a move/load race. Require owner, epoch, state, and live lease; hold/revalidate the grant for the link lifetime; close the link on ownership loss; and prove a stale/non-owner open leaves loader and handler counters at zero.
...
cargo test -p lattice-placement --test authority_mtls
