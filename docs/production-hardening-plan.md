...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven API/backend isolation: raw etcd transport and floor codecs are crate-private with compile-fail coverage; authenticated etcd configuration is bounded and redacted; disposable real-etcd RBAC tests deny runtime actor/floor mutation and prove credential revocation. The public ServiceContext placement view is read/watch-only. Production actor/singleton activation, instance registration/renewal/state transition, and drain cross `PlacementAuthority`; create-only registration allocates its lease inside the backend, rejects an occupied reusable instance key, and revokes a losing etcd lease. Backend CAS also rejects lifecycle regression outside `Starting -> Ready -> Draining/Stopping -> Dead` (with idempotent repeats). The service's erased read-store handle exposes no instance registration, renewal, or state mutation methods. Only the named development adapter restores an in-process authority. Direct singleton-owner renewal remains on the private store handle as called out below.
  ...
  - Reclamation remains disabled. Although singleton ownership and the current direct renewal filter are boot-incarnation-bound, singleton-owner renewal still uses the private store, and the runtime read/watch etcd connection still possesses protocol-level LeaseGrant/LeaseKeepAlive capability even though instance lifecycle call sites now use semantic authority. Rust type narrowing alone is insufficient, so reads/watches require a semantic proxy or equivalent network/L7 Lease denial before the direct credential can be removed. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
...
- [ ] Finish and prove the dedicated singleton owner-lease lifecycle. Records and the current direct keepalive loop are boot-incarnation-bound and in-memory coverage proves prior-boot leases are not renewed; move renewal through authenticated semantic authority, add real-etcd renewal/expiry/reincarnation coverage, validate keepalive interval versus TTL, stop renewing obsolete records, and clean up newly granted leases when reserve, control, or commit fails while preserving monotonic epochs.
