...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it. Hardened actor, virtual-shard, and singleton reserve/raw-CAS paths now reject a live record whose floor lineage is missing or inconsistent before mutating either key, so a visible record-only legacy replay cannot be laundered into a valid floor. Reclamation remains disabled: ownership snapshots and watches do not yet carry a non-leased floor proof from the record's exact source revision, and the exclusive old-writer cutover/backfill gate is not yet enforceable. `LocalOwnershipSnapshot` therefore still retains every tombstone across resync and intentionally fails closed at capacity; do not evict one merely because its placement record is absent or because a later floor value happens to exist.
...
- [ ] Define and test the unsafe placement upgrade preflight: stop old writers and make their credentials/protocol incapable of writing the hardened namespace; reject startup or clean/migrate slash-bearing keys; atomically CAS-backfill every live actor, virtual-shard, and singleton record with its durable floor at one modification revision before the record can disappear; reject or migrate leased floors and prohibit their later deletion; and seed or prohibit reuse of already-deleted historical identities whose last epoch cannot be reconstructed. Do not claim rolling compatibility with ambiguous keys or writers that do not maintain floors.
