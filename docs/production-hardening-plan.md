...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven API/backend isolation: raw etcd transport and floor codecs are crate-private with compile-fail coverage; authenticated etcd configuration is bounded and redacted; disposable real-etcd RBAC tests deny runtime actor/floor mutation and prove credential revocation. The public ServiceContext placement view is read/watch-only; production actor/singleton activation and drain cross PlacementAuthority, and only the named development adapter restores an in-process coordinator for those operations. Private service lifecycle code still holds the direct liveness store handle called out below.
  ...
  - Reclamation remains disabled. Runtime instance/singleton liveness still uses a direct etcd connection and is not isolated behind incarnation-bound semantic authority. Merely narrowing Rust call sites is insufficient because that read connection can still invoke unrestricted LeaseGrant/LeaseKeepAlive, so reads/watches require a semantic proxy or equivalent protocol-level Lease denial. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
