...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Reclamation remains disabled. Authenticated semantic point reads and a bounded, coherent, proof-carrying per-service snapshot now exist, but no-gap watch streaming is not implemented and neither path is wired into service assembly. The concrete runtime read/watch etcd connection therefore still possesses protocol-level LeaseGrant/LeaseKeepAlive capability and could bypass the Rust surface through another client. Type narrowing and partial proxy coverage are insufficient; all reads/watches require the semantic proxy or equivalent network/L7 Lease denial before the direct credential can be removed. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
