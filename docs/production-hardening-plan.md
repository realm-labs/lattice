...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven API/backend isolation: raw etcd transport and floor codecs are crate-private with compile-fail coverage; authenticated etcd configuration is bounded and redacted; disposable real-etcd RBAC tests deny runtime actor/floor mutation and prove credential revocation. Service assembly and the public ServiceContext placement view depend on the read/watch-only `PlacementReadStore`; actor and singleton resolvers require the narrower `PlacementRoutingStore`. `TonicPlacementRoutingStore` implements that contract exclusively through authenticated semantic point reads and the no-gap ownership stream, without list or backend methods, and `placement_routing_store` makes generated clients prefer it. Singleton renewal discovery is split behind `SingletonClaimReader`; the tonic reader derives exact-boot claims from the bounded service snapshot and the keepalive loop prefers `singleton_claim_reader`. Admin reads are split behind `AdminPlacementReader`: service-instance enumeration uses a new `list_instances_bounded` backend operation that requests at most `max + 1` keys from real/deterministic etcd (or takes at most that many under the memory-store lock) and fails before decoding/returning overflow; actor/shard/singleton data comes from the proof-carrying snapshot, and drain lookup uses a semantic point read. `admin_placement_reader` makes initial admin snapshot and mutation lookup bypass the broad store. Executable service coverage proves all three independent components are selected; real mTLS coverage proves routing, watch delivery, exact current-boot claims, bounded two-instance admin snapshots, capacity rejection, and stale identity denial. Production mutations cross `PlacementAuthority`; create-only registration and lifecycle CAS are boot/lease fenced, and singleton renewal exact-validates at most 4,096 complete claims before renewing any lease. The erased broad-store handle exposes no lease mutation. Only the named development adapter restores an in-process authority.
  ...
  - Reclamation remains disabled. Semantic routing, claim-reader, and admin-reader assembly slots now exist, but all are optional and no bundled production bootstrap requires them. The broad ServiceContext placement view and ownership lifecycle remain on the broad store. The concrete runtime read/watch etcd connection therefore still possesses protocol-level LeaseGrant/LeaseKeepAlive capability and could bypass the Rust surface through another client. A partial proxy is insufficient; every production read consumer must use it and the direct credential must be removed or denied Lease RPC at the network/L7 boundary. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
