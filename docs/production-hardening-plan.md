...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven API/backend isolation: raw etcd transport and floor codecs are crate-private with compile-fail coverage; authenticated etcd configuration is bounded and redacted; disposable real-etcd RBAC tests deny runtime actor/floor mutation and prove credential revocation. Service assembly, route resolvers, and the public ServiceContext placement view now depend on the read/watch-only `PlacementReadStore`; `ReadOnlyPlacementStore` deliberately does not implement the writable `PlacementStore`, and executable lifecycle coverage starts, readies, and cleanly stops a service through that wrapper while a separately supplied authority owns mutation. The authenticated coordinator now also exposes bounded semantic instance, actor, and singleton point reads plus one no-gap service ownership stream. The stream opens the backend ownership view once, sends its coherent initial snapshot as the first frame, and then forwards ordered progress and same-revision batches through a 16-frame backpressure channel; backend lag, capacity, startup backlog, compaction revisions, cancellation, protocol, and proof failures terminate through a structured bounded failure frame. Snapshot and watch records carry context-specific floor proof envelopes. The client reconstructs opaque snapshot/upsert/delete proofs only through the lineage-checking constructor, revalidates their exact record binding and revision, rejects repeated snapshots and malformed/future/oversized batches, and returns the native cancellable `OwnershipView`. Real mTLS coverage proves current-boot cross-service snapshot-to-watch delivery with no separate subscribe gap, backend-enforced capacity rejection, and denial of every read/watch to missing, malformed, wrong-domain, wrong-workload, and stale-incarnation identities. Production actor/singleton activation, instance registration/renewal/state transition, bounded singleton-owner renewal, and drain cross `PlacementAuthority`; create-only registration allocates its lease inside the backend, rejects an occupied reusable instance key, and revokes a losing etcd lease. Backend CAS also rejects lifecycle regression outside `Starting -> Ready -> Draining/Stopping -> Dead` (with idempotent repeats). Singleton renewal accepts at most 4,096 boot-bound complete-record claims, rejects duplicate keys/leases and stale or changed records before renewing any lease, and returns an identity-checked exact count. The service's erased read-store handle exposes no lease grant/keepalive, instance registration, or state mutation methods. Only the named development adapter restores an in-process authority.
  ...
  - Reclamation remains disabled. Authenticated semantic point reads and a bounded, proof-carrying no-gap ownership stream now exist, but service assembly, route-cache watching, admin reads, and singleton claim discovery are not cut over. The concrete runtime read/watch etcd connection therefore still possesses protocol-level LeaseGrant/LeaseKeepAlive capability and could bypass the Rust surface through another client. A partial proxy is insufficient; every production read consumer must use it and the direct credential must be removed or denied Lease RPC at the network/L7 boundary. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
