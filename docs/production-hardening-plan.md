...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven authority transport: the standalone coordinator now fails closed unless it has complete bounded mTLS material or an explicit loopback-only plaintext escape. In production mTLS mode, every method authenticates one strict certificate-derived SPIFFE workload identity before semantic request handling or placement mutation; drain additionally requires the exact certificate service/instance and expected lease. Real tonic coverage rejects missing/untrusted certificates, wrong server trust/host, plaintext, malformed or wrong-domain identities, and a spoofed drain target without advancing the placement revision. It also proves an authenticated cross-service activation and exact-identity self-drain. Generic TLS Debug output redacts certificate and private-key bytes.
  - Reclamation remains disabled. Runtime instance/singleton liveness is still direct, unbounded, and not boot-incarnation-bound; the certificate identity ends at a reusable instance ID. Merely narrowing Rust call sites is insufficient because a runtime direct etcd read connection can still invoke unrestricted LeaseGrant/LeaseKeepAlive, so reads/watches require a semantic proxy or equivalent protocol-level Lease denial. Coordinator-to-runtime LogicControl is still plaintext; production RBAC provisioning is not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
