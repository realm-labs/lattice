...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven foundations: hardened actor, virtual-shard, and singleton writers reject missing or inconsistent floor lineage; snapshots and watch events require exact revision-bound floor proofs; compaction, malformed/miskeyed/leased floors, duplicate selected keys, proof mismatch, and capacity overflow fail closed without partial publication. Local state distinguishes present, lifecycle-fenced, exact-delete absent, and coherent-snapshot absent observations, and every absence still consumes capacity.
  ...
  - Reclamation remains disabled. Singleton-owner renewal still uses the private store, and the runtime read/watch etcd connection still possesses protocol-level LeaseGrant/LeaseKeepAlive capability even though instance lifecycle call sites now use semantic authority. Rust type narrowing alone is insufficient, so reads/watches require a semantic proxy or equivalent network/L7 Lease denial before the direct credential can be removed. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
