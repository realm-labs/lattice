...
- [ ] Reclaim retained local ownership tombstones without permanently consuming bounded snapshot capacity only after proving the corresponding durable floor is present and all active writers enforce it.
  ...
  - Proven API/backend isolation: raw etcd transport and floor codecs are crate-private with compile-fail coverage; authenticated etcd configuration is bounded and redacted; disposable real-etcd RBAC tests deny runtime actor/floor mutation and prove credential revocation. Service assembly and the public ServiceContext placement view depend on the read/watch-only `PlacementReadStore`; actor and singleton resolvers require the narrower `PlacementRoutingStore`. `TonicPlacementRoutingStore` implements that contract exclusively through authenticated semantic point reads and the no-gap ownership stream, without list or backend methods, and `placement_routing_store` makes generated clients prefer it. Singleton renewal discovery is split behind `SingletonClaimReader`; the tonic reader derives exact-boot claims from the bounded service snapshot and the keepalive loop prefers `singleton_claim_reader`. Admin reads are split behind `AdminPlacementReader`: service-instance enumeration is backend-bounded at `max + 1`, actor/shard/singleton data comes from the proof-carrying snapshot, and drain lookup uses a semantic point read. The new `OwnershipViewReader` is a fourth narrow capability. When configured, service assembly installs a bounded `LocalOwnershipGate`, opens its coherent view only after the exact boot is published Ready, applies ordered batches to the local snapshot, fences on stream/proof failure, and invalidates the local lease before returning a failed instance keepalive. The readiness signal is not published until the initial view is installed. In-memory lifecycle coverage proves a missing placement is rejected from ready local state and a later hardened ownership commit becomes authorized through the watch. Real mTLS coverage already proves the same semantic stream's admission, bounded snapshot, ordered delivery, and stale denial. Production mutations cross `PlacementAuthority`; create-only registration and lifecycle CAS are boot/lease fenced, and singleton renewal exact-validates at most 4,096 complete claims before renewing any lease. The erased broad-store handle exposes no lease mutation. Only the named development adapter restores an in-process authority.
  ...
  - Reclamation remains disabled. Semantic routing, claim, admin, and ownership-view assembly slots now exist, but all are optional, no bundled production bootstrap requires them, and generated ingress does not yet consume the installed gate. The broad ServiceContext placement view also retains the concrete read/watch etcd connection and its protocol-level LeaseGrant/LeaseKeepAlive capability. A partial proxy is insufficient; every production read consumer must use it and the direct credential must be removed or denied Lease RPC at the network/L7 boundary. Coordinator-to-runtime LogicControl is still plaintext; certificate reload/revocation and production RBAC provisioning are not bundled; authority WRITE can still delete floors; leadership/generation fencing, exclusive legacy-writer revocation, atomic backfill, historical retirement, a protected sealed manifest, permanent floor retention, and an opaque backend-issued reclamation capability do not exist. LocalOwnershipSnapshot therefore intentionally retains every absence and fails closed at capacity; a configuration boolean is insufficient.
